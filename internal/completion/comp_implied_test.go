...
func TestCompletion_BooleanWithDefaultDeduplicated(t *testing.T) {
	...
	body := "enabled: \n"
	list := completion.At(yamlast.Parse([]byte(body)), protocol.Position{Line: 0, Character: 9}, sch)
	if list == nil {
...
func TestCompletion_DefaultValueOffered(t *testing.T) {
	...
	body := "level: \n"
	list := completion.At(yamlast.Parse([]byte(body)), protocol.Position{Line: 0, Character: 7}, sch)
	if list == nil {
